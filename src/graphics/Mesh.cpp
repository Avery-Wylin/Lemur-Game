...
void Mesh::append_mesh( const Mesh &src_mesh, uint32_t src_partition ) {
    ...
    if( src_partition >= src_mesh.partitions.size() ) {
        puts( "Mesh can not be appended, partition does not exist." );
        fflush( stdout );
        return;
    }

    const Partition &p = src_mesh.partitions[src_partition];
...
void Mesh::append_mesh_transformed( Mesh &src_mesh,  uint32_t src_partition, mat4 transform) {
    // Copy the format if there are no partitions
    if(partitions.empty()){
        copy_attribute_format(src_mesh);
    }
...
void Mesh::to_VAO( VAO *vao, uint32_t partition_first, uint32_t partition_last ) {
    ...
    vao->loadIndex( indices.size(), indices.data() );
}
...
AABB Mesh::get_bounding_box( uint32_t partition){
    AABB box;
    ...
    vec3 min,max;
    pos.get_min_max(min,max, partitions[partition].vertex_begin, partitions[partition].vertex_end);
    box.set(min,max);
    return box;
}
