...
void Armature::update() {
    for( AnimationPlayData &p : playing_animations ) {
        p.current_time += ARMATURE_CONSTRAINT_TIMESTEP * p.playback_speed;
    }
...
void Armature::set_root_transform(vec3 pos, versor rot){
    ...
    // glm_quat_mat4(rot, transform_buffer[0]);
    // glm_vec3_copy(pos, transform_buffer[0][3]);
    // glm_mat4_copy(transform_buffer[0],joints[0].transform);
    // glm_mat4_identity(joints[0].transform);
    // glm_mat4_identity(joints[0].inverse_rest);
}
...
void Armature::update_softbody_positions( SoftbodyConstraint &j ) {
    ...
    bool bouyant = head[1] < bouyancy_height;
    vec3 grav = {0, -j.gravity*(bouyant?-1:1), 0};

    // Update tail position
    j.tail_pos[0] += (( 1 - j.drag) * abs_vel[0] + ( 1 - j.friction) * local_vel[0])* (bouyant?0.5f:1.0f) + grav[0] * ARMATURE_CONSTRAINT_TIMESTEP * ARMATURE_CONSTRAINT_TIMESTEP;
    j.tail_pos[1] += (( 1 - j.drag) * abs_vel[1] + ( 1 - j.friction) * local_vel[1])* (bouyant?0.5f:1.0f) + grav[1] * ARMATURE_CONSTRAINT_TIMESTEP * ARMATURE_CONSTRAINT_TIMESTEP;
    j.tail_pos[2] += (( 1 - j.drag) * abs_vel[2] + ( 1 - j.friction) * local_vel[2])* (bouyant?0.5f:1.0f) + grav[2] * ARMATURE_CONSTRAINT_TIMESTEP * ARMATURE_CONSTRAINT_TIMESTEP;
...
void ArmatureInfo::load( string filename ) {
