...
void Shader::linkDefaultUniforms() {
    ...
    uniform_locations[UNIFORM_WATER] = glGetUniformLocation( program_id, "water" ) ;
    uniform_locations[UNIFORM_FOG] = glGetUniformLocation( program_id, "fog_data" ) ;
    uniform_locations[UNIFORM_SHAPEKEY] = glGetUniformLocation( program_id, "shapekey_factor" ) ;
...
void Shader::bind(Shader &shader) {
    ...
    if( glIsProgram( active->program_id ) == GL_TRUE ){
        ...
        printf( "Shader has invalid program ID: %i\n", active->program_id );
        exit(1);
    }
}
