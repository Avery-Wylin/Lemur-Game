...
class Attribute_Data {
    ...
    public:
        ...
        void load_vbo( uint8_t attribute, VAO *vao, bool byte_to_float = false ) {
            ...
            if( data_type == FLOAT ) {
                vao->loadAttributeFloat( attribute, 0, 0, vector_size, data_float.size(), data_float.data() );
            }
            else if( data_type == BYTE ) {
                vao->loadAttributeByte( attribute, 0, 0, vector_size, data_byte.size(), byte_to_float, data_byte.data() );
            }
        }
...
class Mesh {
    ...
    public:
        ...
        void append_mesh( const Mesh &src_mesh, uint32_t src_partition );
        void append_mesh_transformed( Mesh &src_mesh,  uint32_t src_partition, mat4 transform );
        ...
        AABB get_bounding_box( uint32_t partition);
};
