 ...
 */
void VAO::loadAttributeFloat( int attrbid, int vertexOffset, int divisor, int vecSize, int size, void *data ) {
    ...
    if( offset > 0 ) {
        glBufferSubData( GL_ARRAY_BUFFER, offset, std::min( datasize, vboSizes[attrbid] - offset ), data );
    }
    else {
...
void VAO::loadAttributeByte( int attrbid, int vertexOffset, int divisor, int vecSize, int size,  bool convert_float, void *data ) {
...
void VAO::loadIndex( int numIndices, GLuint *data ) {
    if( vaoid != 0 ) {
        glBindVertexArray( vaoid );
        if( iboid == 0 ) {
...
void VAO::loadPLY( std::string filename ) {
    ...
    if( posCount > 0 ) {
        this->loadAttributeFloat( Attribute::ATTRB_POS, 0, 0, posCount, pos.size(), pos.data() );
    }
    if( normalCount > 0 ) {
        this->loadAttributeFloat( Attribute::ATTRB_NORM, 0, 0, normalCount, norm.size(), norm.data() );
    }
    if( uvcount > 0 ) {
        this->loadAttributeFloat( Attribute::ATTRB_UV, 0, 0, uvcount, uv.size(), uv.data() );
    }
    if( colorCount > 0 ) {
        this->loadAttributeByte( Attribute::ATTRB_COL, 0, 0, colorCount, col.size(), true, col.data() );
    }
    if( weightCount > 0 ) {
        this->loadAttributeFloat( Attribute::ATTRB_WEIGHTS, 0, 0, weightCount, weight.size(), weight.data() );
    }
    if(jointCount > 0 ){
        this->loadAttributeByte(Attribute::ATTRB_JOINTS, 0, 0, jointCount, joint.size(), false, joint.data());
    }

    this->loadIndex( faces.size(), faces.data() );
}
