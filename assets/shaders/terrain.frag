...
vec3 diff_edge = vec3(0.21,0.15,0.1);
vec3 diff_flat = vec3(0.59,0.45,0.27);
...
void main(void){
    ...
    vec3 diff = dot(n,vec3(0,1,0)) > .9 ? diff_flat : diff_edge;
    color_out.rgb = (zenith + horizon * pow(1-max(dot(n, vec3(0,1,0)),0),2) + f * sun) * diff;
